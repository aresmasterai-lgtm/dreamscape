...
function DreamWidget({ user, onSignIn }) {
  ...
  return (
    <>
      ...
      `}</style>
      {open && (
        <div style={{ position: 'fixed', bottom: 90, right: 20, zIndex: 9000, width: 340, maxHeight: 500, background: C.card, border: `1px solid ${C.accent}55`, borderRadius: 20, display: 'flex', flexDirection: 'column', boxShadow: `0 8px 60px rgba(124,92,252,0.3), 0 2px 20px rgba(0,0,0,0.5)`, animation: 'widgetIn 0.3s cubic-bezier(0.16,1,0.3,1)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '10px 12px', borderTop: `1px solid ${C.border}`, flexShrink: 0, background: C.panel }}>
            {!user ? (
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && send()} placeholder="Ask Dream anything..." style={{ flex: 1, background: C.bg, border: `1px solid ${C.border}`, borderRadius: 10, padding: '9px 12px', color: C.text, fontSize: 13, outline: 'none', fontFamily: 'inherit' }} />
